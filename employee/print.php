                ...
                <div class="col-lg-6">
                    <div class="card" id="document">
                        <div class="card-block">
                            ...
                            try
                            {
                                $sql = "Select a.QRCode,c.Fullname,c.EmailAddress,c.Contact,d.RoomNumber,FORMAT(IFNULL(e.Rate,0),2)Rate,FORMAT(IFNULL(f.TotalCharge,0),2)total,
                                FORMAT(IFNULL(f.Pay_Amount,0),2) pay,FORMAT(IFNULL(f.Change_Amount,0),2) as balance,f.Reference,f.Date,b.FromDate,b.ToDate,f.Month from tblpayment a 
                                LEFT JOIN tbltransaction b ON b.trxnID=a.trxnID 
                                LEFT JOIN tblcustomer c ON c.customerID=b.customerID 
                                LEFT JOIN tblavailableroom d ON d.aID=b.aID 
                                LEFT JOIN tblrooms e ON e.roomID=d.roomID
                                LEFT JOIN (Select * from tbl_bill ORDER BY billID DESC LIMIT 1)f ON f.QRCode=a.QRCode WHERE a.QRCode=:code";
                                $stmt = $dbh->prepare($sql);
                                $stmt->bindParam(':code',$trxn);
                                $stmt->execute();
                                $data = $stmt->fetchAll();
                                foreach($data as $row)
                                {
                                    ...
                                    <div class="form-group table-responsive">
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                ...
                                                <tr><td colspan='2'>Balance</td><td style="text-align:right;">PHP <?php echo $row['balance'] ?></td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <p>Scan here : </p>
